...
מגבלת התכנון המרכזית מחמירה יותר מפריסה רספונסיבית רגילה: כל עוד המשתמש לא בחר במפורש ריבוי עמודים, כל הטבלה חייבת להימצא בעמוד פיזי יחיד בגודל A4 או A3 ובכיוון שנבחר. כל התאים חייבים להישאר שווי-רוחב ושווי-גובה. אם אי אפשר להכניס את התוכן במלואו, היישום חייב להזהיר או לחסום ייצוא, ולא להסתיר טקסט.

המוצר נמסר כיישום מודולרי המבוסס על Next.js App Router ונפרס ב־Vercel. אין חשבון משתמש או מסד נתונים פרטי. נתיב שרת מאמת ומתווך את הבקשות ל־Hebcal, ואילו ההגדרות, האזכורים האישיים ותשובות אחרונות לטווחים מדויקים נשארים מקומית בדפדפן.
...
4. להתאים את כל הטבלה לעמוד A4 או A3 יחיד, אנכי או אופקי, כברירת מחדל.
...
7. **תצוגת שבוע במובייל** - שבעה כרטיסי יום קריאים, ניווט בין שבועות ועריכה ישירה של אזכור.
8. **סרגל פעולות נייד** - גישה באזור האגודל להגדרות, שבוע, דף פיזי וייצוא PDF.

### 7.2 התנהגות רספונסיבית

- במחשב מוצג לוח הגדרות קבוע לצד אזור תצוגה נגלל.
- מתחת ל־1024px ההגדרות הופכות למגירת צד מודאלית; ב־840px ומטה הן הופכות למסך מלא עם סעיפי אקורדיון.
- ב־840px ומטה ברירת המחדל היא תצוגת שבוע מותאמת למגע ובה בדיוק שבעה כרטיסי יום ופקדי ניווט בגודל 48px.
- סרגל תחתון קבוע ומודע ל־safe areas מספק הגדרות, שבוע קריא, דף מלא וייצוא PDF.
- במסך אופקי ברוחב 600–840px, ובמסך אנכי ברוחב 720–840px, כרטיסי הימים מוצגים בשתי עמודות.
- תצוגת הדף הפיזי המדויקת נשארת זמינה כמצב נפרד והזום שלה מוקטן אוטומטית כאשר הדף חורג מן השטח הפנוי.
...
- בזמן תצוגת השבוע הדף הפיזי נשאר מרונדר לצורך בדיקת גלישה, אך הוא inert ומוסתר מטכנולוגיות מסייעות. ההדפסה משתמשת תמיד בדף הפיזי ולא בכרטיסי המובייל.
...
| גודל נייר | A4 ‏210×297 מ״מ, A3 ‏297×420 מ״מ | A4 | מידות CSS פיזיות במ״מ |
| כיוון | אנכי, אופקי | אנכי | מחליף בין הרוחב והגובה הפיזיים |
...
המערכת היא יישום Next.js מודולרי. Vercel מגיש את הממשק הסטטי ואת נתיב השרת הצר שמאמת ומתווך בקשות ל־Hebcal. תוכן אישי נשאר בדפדפן בלבד.

```mermaid
flowchart LR
    U[משתמש] --> UI[הגדרות RTL ותצוגה]
    UI --> V[אימות טווח והגדרות]
    V --> BFF[נתיב לוח ב-Next.js]
    BFF --> API[Hebcal REST API]
    API --> N[נרמול שרת לפי תאריך ISO]
    N --> R[מנוע רינדור הלוח]
    LS[(הגדרות ב-localStorage)] <--> UI
    IDB[(אזכורים ומטמון ב-IndexedDB)] <--> UI
...
- Next.js 16 App Router ו־React 19 מספקים את גבולות הנתיבים והרכיבים.
- TypeScript במצב Strict מפריד בין תחום, יישום, תשתית, מצב, רכיבים וסגנונות.
- חנויות Zustand חושפות מצב הגדרות ולוח בלי לשלוח אזכורים אישיים לשרת.
- הנתיב `/api/calendar` מאמת קלט בעזרת Zod, פונה ל־Hebcal דרך מתאם שרת ומחזיר חוזה מנורמל.
- הדפדפן שומר הגדרות בגרסה מנוהלת ב־localStorage ואזכורים ומטמון ב־IndexedDB.
...
| רינדור | יצירת עמודים, תאים, כותרות, גבולות CSS ותוויות נגישות |
...
אובייקט ההגדרות כולל טווח, מנהג, נייר, כיוון, חלוקה, מתגי תוכן, טיפוגרפיה, גאומטריה, צבעים וטקסט אירועים חיצוניים. הוא נשמר תחת `seder-yom-settings-v2`.
...
האזכורים מקובצים לפי ISO ונשמרים במאגר IndexedDB ‏`seder-yom-studio/personal-notes`. מנגנון חד־פעמי מייבא את מבנה localStorage הישן.
...
המטמון ממופה לפי הטווח המדויק והמנהג, כולל בכל מטען את הטווח המיושר, ושומר רק את עשר הרשומות החדשות ביותר ב־IndexedDB ‏`seder-yom-studio/calendar-cache`.
...
CSS Grid משתמש בשבע עמודות `minmax(0, 1fr)`, בשורת כותרת מפורשת ובשורות שבוע שוות. תאים ריקים ומלאים משתתפים באותם Tracks. גבולות CSS יציבים להדפסה יוצרים בדיוק:

- שמונה גבולות אנכיים עבור שבע עמודות.
- `rowsPerPage + 2` גבולות אופקיים: עליון, תחתית הכותרת ותחתית לכל שורת שבוע.

הגבולות משתמשים בעובי ובצבע שנבחרו. מנוע הפריסה מאמת את גאומטריית ה־PDF וחוסם ייצוא כאשר נשארה גלישת טקסט.
...
- גבולות הטבלה המבניים נשארים גלויים גם בלי הדפסת רקעים.
...
מחוון הרשת מציג את `navigator.onLine` באופן עצמאי ומפורש, כדי ששגיאת מקור ב־Hebcal לא תסומן בטעות כאובדן חיבור לאינטרנט. סרגל ההודעות מפריד בין נתונים מאומתים, מטמון לטווח מדויק, טעינה ושגיאת מקור.

## 17. שמירה, פרטיות ואבטחה

### 17.1 נתונים מקומיים

היישום שומר הגדרות מנוהלות ב־`localStorage` ואזכורים ומטמון ב־IndexedDB. מחיקת נתוני אתר או שימוש בפרופיל דפדפן אחר עלולים לבודד או למחוק נתונים. לכן יש לייצא JSON לצורך גיבוי.

### 17.2 זרימת מידע אישי

- אזכורים אישיים נשארים מקומיים.
- אזכורים אינם נשלחים ל-Hebcal.
- שרת היישום מקבל רק טווח אזרחי מבוקש ובחירת ישראל/חו״ל; Hebcal מקבל טווח מיושר ופרמטרים של קטגוריות לוח.
...
- אורך אזכור מוגבל ל־500 תווים וגלישה שלא נפתרה חוסמת ייצוא.
...
- גבולות הטבלה המבניים אינם מוסיפים רעש לעץ הנגישות.
...
- שמונה גבולות אנכיים ומספר גבולות אופקיים צפוי.
...
- מספר גבולות הטבלה במסך ובהדפסה.
...
- עמוד בעל שבע עמודות כולל בדיוק שמונה גבולות אנכיים.
...
מריצים `npm install` ולאחר מכן `npm run dev`, ופותחים `http://localhost:3000`. יש לבדוק גם מצב ייצור בעזרת `npm run build` ו־`npm start`.

### 22.2 אחסון סטטי

גרסת הייצור נפרסת בכתובת `https://hebrew-calendar-studio.vercel.app` מפני שהנתיב `/api/calendar` דורש סביבת Next.js. הפרויקט מקושר למאגר GitHub לטובת פריסות חוזרות וניתנות לשחזור. כתובת GitHub Pages הישנה מציגה הפניה מינימלית לאתר הייצור ב־Vercel.
...
8. יש להריץ לינט, בדיקת טיפוסים, בדיקות יחידה, Playwright ובניית ייצור לפני פרסום.
...
- חלון הדפסה יכול לדגום קווים דקים באופן לא עקבי; אימות מטא־נתוני PDF ורינדור PNG הם חלק מבדיקת גרסה.
- זמינות גופן רשת משנה מדדי שורה. הייצוא ממתין ל-`document.fonts.ready`, אך גופן גיבוי עשוי להיראות אחרת.
- אחסון הדפדפן תלוי פרופיל ואינו מערכת גיבוי.
...
1. היישום עובר בניית ייצור של Next.js ונפתח בפריסת Vercel.
...
8. בכל עמוד תאים שווים וגבולות מבניים מלאים.
...
| רשת הדפסה | Tracks של CSS Grid וגבולות יציבים השומרים על תאים מלאים |
